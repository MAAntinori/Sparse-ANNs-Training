...
def create_pdf_report(mean_results, file_path):
    """
    Creates a PDF report summarizing mean results.

    Args:
    - mean_results: Dictionary containing mean results.
    - file_path: File path for saving the PDF report.
    ...
    - None
    """                    
def monte_carlo_simulation(iterations, models, sparsity_levels, epochs_to_try):
    
    results_list = []

    for _ in range(iterations):
        results = train_and_evaluate_models(models, sparsity_levels, epochs_to_try)
    ...
    'Inference Energy Consumption in Watts': np.mean(results_df['Inference Energy Consumption']),
    ...
    pdf_file_path = 'monte_carlo_simulation_results_8x8.pdf'
    create_pdf_report(mean_results, pdf_file_path)
    # Save detailed results to an Excel file
    detailed_results_file_path = 'detailed_monte_carlo_simulation_results_8x8.xlsx'
...
def main():
    ...
    iterations = 5  # You can adjust this value -> runs for MC Simulation
    monte_carlo_simulation(iterations, models, sparsity_levels, epochs_to_try)
