...
- `monte_carlo_simulation`: Conducts a Monte Carlo simulation by repeating the training and evaluation process for a
  specified number of iterations, collecting detailed results in an Excel file and mean results in a PDF report.
- `create_pdf_report`: Creates a PDF report summarizing mean results.
...
def create_cnn():
    ...
    Returns:
    - model: A Keras Sequential model representing the CNN.
    """
...
# Create a transition layer for a DenseNet model
    ...
    Args:
    - x: Input tensor.
    - compression_factor: Compression factor for the transition layers.
...
def create_densenet(input_shape, num_classes, num_layers_per_block, growth_rate, compression_factor):
    ...
    Returns:
    - model: A Keras Model representing the DenseNet.
    """
...
def create_resnet(input_shape, num_classes):
    ...
    Returns:
    - model: A Keras Model representing the ResNet.
    """
...
def evaluate_model(interpreter, data_generator):
    ...
    Args:
    - interpreter: A TFLite interpreter for the model.
    - data_generator: A data generator providing input images and labels.
...
def create_data_generators(image_size=(8, 8),color_mode='rgb',batch_size=16):
    """
    Creates data generators for training, validation, and testing.

    Args:
    - image_size: Tuple specifying the image dimensions (height, width).
    - color_mode: Color mode for image loading ('rgb' or 'grayscale').
...
def calculate_energy_consumption(model, num_epochs, inference_time, power_consumption):
    ...
    - inference_time: Average inference time per frame during testing.
    - power_consumption: Power consumption value during training and inference.

    Returns:
    - training_energy: Energy consumption during training (in watt-hours).
    - inference_energy: Energy consumption during inference (in watt-hours).
    """
...
def train_and_evaluate_models(models,sparsity_levels,epochs_to_try, color_mode_choice):
    """
    Iterates over different combinations of models, sparsity levels, quantization options, and epochs
    to train and evaluate the models. Results are saved to an Excel file.
    ...
    power_consumption_inference = 50  # Adjust this value based on your hardware during inference
     ...
     # Get the memory usage from the profile
    #memory_usage = psutil.Process(os.getpid()).memory_info().rss / 1024 ** 2  # Convert to megabytes
            ...
            for quantize_model in [True, False]:
                for num_epochs in epochs_to_try:
                    ...
                    if model_choice == '1':
                        ...
                        model_name = "DenseNet"
                        model = create_densenet(input_shape=(8, 8, 3), num_classes=1, num_layers_per_block=[4, 4, 4], growth_rate=12, compression_factor=0.5)
                    elif model_choice == '3':
                        model_name = "ResNet"
                        model = create_resnet(input_shape=(8, 8, 3), num_classes=1)
                    else:
                    ...
                    # Compile the model
                    # Replace deprecated loss function
                    ...
                    file_name = 'results_Montecarlo_Gray_scale.xlsx'
...
def create_pdf_report(mean_results, color_mode,image_size,model_choices, file_path):
    ...
    Args:
        mean_results (dict): A dictionary containing mean results.
        color_mode (str): The color mode used for the images ('rgb' or 'grayscale').
        image_size (tuple): A tuple specifying the image dimensions (height, width).
        file_path (str): The file path for saving the PDF report.
    ...
    pdf_canvas.drawCentredString(300, 750, "Monte Carlo Simulation Results")
    
    # Add color mode choice to the PDF and image size to the PDF
    ...
    """
    Conducts a Monte Carlo simulation by repeating the training and evaluation process for a
    specified number of iterations, collecting detailed results in an Excel file and mean results
    in a PDF report.

    Args:
        iterations (int): The number of Monte Carlo iterations to perform.
        models (list): List of models to evaluate.
        sparsity_levels (list): List of sparsity levels to evaluate.
        epochs_to_try (list): List of epochs to try during training.
        color_mode_choice (str): The color mode choice for image processing ('rgb' or 'grayscale').
        image_size (tuple): A tuple specifying the image dimensions (height, width).

    Returns:
        None
    """                    
def monte_carlo_simulation(iterations, models, sparsity_levels, epochs_to_try, color_mode_choice, image_size):
    
    results_list = []

    for _ in range(iterations):
        results = train_and_evaluate_models(models, sparsity_levels, epochs_to_try, color_mode_choice)
    ...
    'Inference Time': np.mean(results_df['Inference Time']),
    'Execution Time in seconds': np.mean(results_df['Execution Time in seconds']),    
    'Inference Energy Consumption in Watts': np.mean(results_df['Inference Energy Consumption in Watts']),
    'Memory Usage in MB': np.mean(results_df['Memory Usage in MB'])
}

    # Print or save the mean results
    print("Mean Results:")
    print(mean_results)
...
def main():
    ...
    # Validate user input
    if color_mode_choice.lower() not in ['rgb', 'grayscale']:
        print("Invalid color mode choice. Please choose 'rgb' or 'grayscale'.")
        return
    
    # Prompt user for image size choice
    image_size_choice = input("Choose image size (8x8, 16x16, or 32x32): ")

    # Validate user input
    if image_size_choice not in ['8x8', '16x16', '32x32']:
        print("Invalid image size choice. Please choose '8x8', '16x16', or '32x32'.")
        return
    # Convert image size choice to a tuple
    image_size = tuple(map(int, image_size_choice.split('x')))

    # Create data generators with the chosen image size
